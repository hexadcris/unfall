...
SystemComponentOutputItemView::SystemComponentOutputItemView(SystemComponentOutputItemInterface *const output,
    ...
    WidgetView(parent), output(output), ui(new Ui::SystemComponentOutputItemView)
{
    ui->setupUi(this);
    ui->title->setText((output->getUnit().isEmpty())
                           ? QString(QStringLiteral("%1")).arg(output->getTitle())
                           : QString(QStringLiteral("%1 [%2]")).arg(output->getTitle(), output->getUnit()));
