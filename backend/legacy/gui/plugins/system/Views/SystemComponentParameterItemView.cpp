...
SystemComponentParameterItemView::SystemComponentParameterItemView(SystemComponentParameterItemInterface *const parameter,
    ...
    WidgetView(parent), parameter(parameter), ui(new Ui::SystemComponentParameterItemViews)
{
    // Setup UI
    ui->setupUi(this);
    updateTitle();
    ...
    if (parameter->getType() == QStringLiteral("bool"))
        ui->layout->addWidget(new SystemComponentParameterValueBoolView(parameter, this), 0, Qt::AlignRight);

    // Setup value / int
    if (parameter->getType() == QStringLiteral("int"))
        ui->layout->addWidget(new SystemComponentParameterValueIntView(parameter, this), 0, Qt::AlignRight);

    // Setup value / float
    if (parameter->getType() == QStringLiteral("float"))
        ui->layout->addWidget(new SystemComponentParameterValueFloatView(parameter, this), 0, Qt::AlignRight);

    // Setup value / double
    if (parameter->getType() == QStringLiteral("double"))
        ui->layout->addWidget(new SystemComponentParameterValueDoubleView(parameter, this), 0, Qt::AlignRight);

    // Setup value / string
    if (parameter->getType() == QStringLiteral("string"))
        ui->layout->addWidget(new SystemComponentParameterValueStringView(parameter, this), 0, Qt::AlignRight);

    // Setup value / string
    if (parameter->getType() == QStringLiteral("normalDistribution"))
        ui->layout->addWidget(new SystemComponentParameterValueNormalDistributionView(parameter, this), 0, Qt::AlignRight);
}
...
void SystemComponentParameterItemView::updateTitle()
{
    ...
    ui->title->setText((parameter->getUnit().isEmpty())
                           ? QString(QStringLiteral("%1")).arg(parameter->getTitle())
                           : QString(QStringLiteral("%1 [%2]")).arg(parameter->getTitle(), parameter->getUnit()));
}
