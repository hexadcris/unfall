...
SystemComponentInputItemView::SystemComponentInputItemView(SystemComponentInputItemInterface *const input,
    ...
    WidgetView(parent), input(input), ui(new Ui::SystemComponentInputItemView)
{
    ui->setupUi(this);
    ui->title->setText((input->getUnit().isEmpty())
                           ? QString(QStringLiteral("%1")).arg(input->getTitle())
                           : QString(QStringLiteral("%1 [%2]")).arg(input->getTitle(), input->getUnit()));
