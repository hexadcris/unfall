...
*
* \details The algorithms for collision detection and calculation of the post-crash dynamics are based
...
*/
//-----------------------------------------------------------------------------
class CollisionDetectionPostCrash
{
public:
    CollisionDetectionPostCrash() = default;
    CollisionDetectionPostCrash(const CallbackInterface *callbacks)
        : callbacks(callbacks)
    {}
    CollisionDetectionPostCrash(const CollisionDetectionPostCrash &) = delete;
    CollisionDetectionPostCrash(CollisionDetectionPostCrash &&) = delete;
    CollisionDetectionPostCrash &operator=(const CollisionDetectionPostCrash
                                                        &) = delete;
    CollisionDetectionPostCrash &operator=(CollisionDetectionPostCrash &&) =
        delete;

    ~CollisionDetectionPostCrash();

    /*!
     * \brief Calculate and store collision data
     * Calculates collision time and post-crash dynamics.
     *
     ...
     * \return                             true if postCrashDynamics are created successfully
     */
    bool CreatePostCrashDynamics(const AgentInterface *agent1,
    ...
    /*!
     * \brief Retrieve collision angles
     *
     * \return                             collision angles
     */
    CollisionAngles GetCollisionAngles();

    /*!
     * \brief Sets collision angles as defined in http://indexsmart.mirasmart.com/26esv/PDFfiles/26ESV-000177.pdf
     *
     * \param[in]  OYA                  opponent yaw angle relative to host yaw angle
     * \param[in]  HCPAo                original host collision point angle
     * \param[in]  OCPAo                original opponent collision point angle
...
private:
    ...
    /*!
     * \brief Gets corners of one agent
     ...
     * \param[out] firstTimeContact      time of first contact --> negative since in the past [ms]
     * \return                           flag indicating if calculation could be done correctly
     */
    bool GetFirstContact(const AgentInterface *agent1, const AgentInterface *agent2,
                               int &timeFirstContact);

    /*!
     * \brief calculates collision angles
     * The time of the first contact of two colliding agents is used to calculate the
     * first point of contact. Based on this first point of contact, the collision angles
     * are calculated.
     *
     * \param[in] agent1                 first agent to consider
     * \param[in] agent2                 other agent to consider
     * \param[in] firstTimeContact       time of first contact --> negative since in the past [ms]
     */
    void CalculateCollisionAngles(const AgentInterface *agent1,
                                  const AgentInterface *agent2,
                                  int timeFirstContact);
    ...
    CollisionAngles collAngles; //!< collision angles [degrees] as defined in http://indexsmart.mirasmart.com/26esv/PDFfiles/26ESV-000177.pdf

    int penetrationTime = 30; //!< time for overlapping vehicles [ms]
    /** @} */
